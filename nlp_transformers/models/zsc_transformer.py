...
class ZeroShotClassificationTransformer(BaseTransformer, TrainingMixin):
    ...
    def _create_sequence_pairs(self, sequences, labels, hypothesis_template):
        ...
        # the procedure creates new records
        # where each label is paired with each sequence
    ...
    def _postprocess_logits(
        self, logits: np.ndarray, no_sequences: int, no_labels: int, multi_label: bool
    ):
        ...
        if not multi_label:
            ...
            # softmax over the entailment vs. contradiction dim
    ...
    def predict_sample(
        ...
        *,
        ...
        sequence_pairs = self._create_sequence_pairs(
            sequences, labels, hypothesis_template
        )
        ...
        probs = self._postprocess_logits(
            logits, len(sequences), len(labels), multi_label
        )
    ...
    def predict(
        ...
        *,
        ...
        probs = self._postprocess_logits(
            out.predictions, no_sequences, len(labels), multi_label
        )

        return ZeroShotClassificationOutput(
            probs=probs, metrics=getattr(out, "metrics", None)
        )
